<template>
  <view class="place-detail pageBg">
    ...
    <view class="card" v-if="profile">
      <view class="section-head">
        <text class="section-title">KTV信息</text>
      </view>
      <view class="row">
        <text class="label">营业执照</text>
        <text class="value">{{ profile.primary.businessLicenseNo }}</text>
      </view>
      <view class="row">
        <text class="label">执照到期</text>
        <text :class="['value', dueClass(profile.primary.businessLicenseDue)]">{{ profile.primary.businessLicenseDue }}</text>
      </view>
      <view class="row">
        <text class="label">特行许可</text>
        <text class="value">{{ profile.primary.specialLicenseNo }}</text>
      </view>
      <view class="row">
        <text class="label">许可到期</text>
        <text :class="['value', dueClass(profile.primary.specialLicenseDue)]">{{ profile.primary.specialLicenseDue }}</text>
      </view>
      <view class="row">
        <text class="label">消防检查</text>
        <text class="value">{{ profile.primary.fireCheckDate }}</text>
      </view>
      <view class="row"><text class="label">包厢数</text><text class="value">{{ profile.primary.roomCount }}</text></view>
      <view class="row"><text class="label">营业时间</text><text class="value">{{ profile.primary.businessHours }}</text></view>
      <view class="row"><text class="label">安保人数</text><text class="value">{{ profile.primary.securityCount }}</text></view>
      <view class="row">
        <text class="label">风险标记</text>
        <text class="value">{{ (profile.primary.riskFlags || []).join(' / ') }}</text>
      </view>
    </view>
...
function moduleSummary(type) {
  ...
  if (type === 'CHESS_CARD') return `麻将台 ${data.mahjongTableCount || 0} / 包间 ${data.chessRoomCount || 0}`;
